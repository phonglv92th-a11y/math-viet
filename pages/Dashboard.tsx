...
import { Brain, Puzzle, ShoppingCart, Shapes, PlayCircle, Zap, Palette, Swords, Search, BookOpen, PenTool, Hammer, ScanEye, X, ChevronRight, Trophy, Loader2, Layers, Feather, Quote, Globe, Languages, Type, MessageCircle } from 'lucide-react';
  ...
  const renderGameCards = () => {
    switch(subject) {
      case SubjectEnum.MATH:
        return (
          <>
             <GameCard title="Tính Nhẩm" description="Cộng trừ nhân chia" icon={Brain} color="blue" type={GameType.MENTAL_MATH} mode={gameMode} stats={user.progress[GameType.MENTAL_MATH]} customStyle={gameStyles[GameType.MENTAL_MATH]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.MENTAL_MATH, mode: gameMode })} />
             <GameCard title="Mật Mã Logic" description="Tìm quy luật số" icon={Puzzle} color="purple" type={GameType.LOGIC_PUZZLE} mode={gameMode} stats={user.progress[GameType.LOGIC_PUZZLE]} customStyle={gameStyles[GameType.LOGIC_PUZZLE]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.LOGIC_PUZZLE, mode: gameMode })} />
             <GameCard title="Toán Thực Tế" description="Đi chợ, tính tiền" icon={ShoppingCart} color="green" type={GameType.REAL_WORLD} mode={gameMode} stats={user.progress[GameType.REAL_WORLD]} customStyle={gameStyles[GameType.REAL_WORLD]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.REAL_WORLD, mode: gameMode })} />
             <GameCard title="Đếm Hình" description="Đếm vật thể vui nhộn" icon={Shapes} color="teal" type={GameType.VISUAL_COUNT} mode={gameMode} stats={user.progress[GameType.VISUAL_COUNT]} customStyle={gameStyles[GameType.VISUAL_COUNT]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.VISUAL_COUNT, mode: gameMode })} />
             <GameCard title="Xây Tháp Trí Tuệ" description="Sắp xếp số, điền số" icon={Layers} color="pink" type={GameType.TOWER_STACK} mode={gameMode} stats={user.progress[GameType.TOWER_STACK]} customStyle={gameStyles[GameType.TOWER_STACK]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.TOWER_STACK, mode: gameMode })} />
          </>
        );
      case SubjectEnum.LITERATURE:
        return (
          <>
             <GameCard title="Vua Tiếng Việt" description="Ghép từ, đồng nghĩa" icon={PenTool} color="rose" type={GameType.WORD_MATCH} mode={gameMode} stats={user.progress[GameType.WORD_MATCH]} customStyle={gameStyles[GameType.WORD_MATCH]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.WORD_MATCH, mode: gameMode })} />
             <GameCard title="Chính Tả" description="Sửa lỗi sai" icon={Search} color="cyan" type={GameType.SPELLING_BEE} mode={gameMode} stats={user.progress[GameType.SPELLING_BEE]} customStyle={gameStyles[GameType.SPELLING_BEE]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.SPELLING_BEE, mode: gameMode })} />
             <GameCard title="Thợ Xây Câu" description="Sắp xếp câu đúng ngữ pháp" icon={Hammer} color="orange" type={GameType.SENTENCE_BUILDER} mode={gameMode} stats={user.progress[GameType.SENTENCE_BUILDER]} customStyle={gameStyles[GameType.SENTENCE_BUILDER]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.SENTENCE_BUILDER, mode: gameMode })} />
             <GameCard title="Thám Tử Văn Học" description="Biện pháp tu từ, ẩn dụ" icon={ScanEye} color="indigo" type={GameType.LITERARY_DETECTIVE} mode={gameMode} stats={user.progress[GameType.LITERARY_DETECTIVE]} customStyle={gameStyles[GameType.LITERARY_DETECTIVE]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.LITERARY_DETECTIVE, mode: gameMode })} />
             <GameCard title="Nhà Thơ Tài Ba" description="Điền từ vào thơ" icon={Feather} color="yellow" type={GameType.POETRY_PUZZLE} mode={gameMode} stats={user.progress[GameType.POETRY_PUZZLE]} customStyle={gameStyles[GameType.POETRY_PUZZLE]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.POETRY_PUZZLE, mode: gameMode })} />
      ...
      case SubjectEnum.ENGLISH:
        return (
          <>
             <GameCard title="Vua Từ Vựng" description="Vocabulary & Meaning" icon={Globe} color="indigo" type={GameType.ENGLISH_VOCAB} mode={gameMode} stats={user.progress[GameType.ENGLISH_VOCAB]} customStyle={gameStyles[GameType.ENGLISH_VOCAB]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.ENGLISH_VOCAB, mode: gameMode })} />
             <GameCard title="Ngữ Pháp" description="Grammar & Tenses" icon={Languages} color="violet" type={GameType.ENGLISH_GRAMMAR} mode={gameMode} stats={user.progress[GameType.ENGLISH_GRAMMAR]} customStyle={gameStyles[GameType.ENGLISH_GRAMMAR]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.ENGLISH_GRAMMAR, mode: gameMode })} />
             <GameCard title="Đánh Vần" description="Spelling Challenge" icon={Type} color="fuchsia" type={GameType.ENGLISH_SPELLING} mode={gameMode} stats={user.progress[GameType.ENGLISH_SPELLING]} customStyle={gameStyles[GameType.ENGLISH_SPELLING]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.ENGLISH_SPELLING, mode: gameMode })} />
             <GameCard title="Đố Vui Tiếng Anh" description="General Knowledge" icon={MessageCircle} color="sky" type={GameType.ENGLISH_QUIZ} mode={gameMode} stats={user.progress[GameType.ENGLISH_QUIZ]} customStyle={gameStyles[GameType.ENGLISH_QUIZ]} onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.ENGLISH_QUIZ, mode: gameMode })} />
          </>
        )
    }
  }
           ...
           <div>
                 ...
                 <div className="flex items-center gap-2 flex-wrap">
                     ...
                     <div className="bg-gray-100 p-1 rounded-lg flex text-xs font-bold">
                        <button 
                           ...
                           className={`px-3 py-1.5 rounded-md transition-all flex items-center ${gameMode === GameMode.STANDARD ? 'bg-white shadow text-gray-800' : 'text-gray-500'}`}
                        >
                            Cơ bản
                        </button>
                        <button 
                           onClick={() => setGameMode(GameMode.SPEED_RUN)}
                           className={`px-3 py-1.5 rounded-md transition-all flex items-center ${gameMode === GameMode.SPEED_RUN ? 'bg-gradient-to-r from-red-500 to-orange-500 shadow text-white' : 'text-gray-500 hover:text-red-500'}`}
                        >
                     ...
                     <button onClick={openStyleModal} className="p-2 hover:bg-gray-100 rounded-lg text-gray-500" title="Tùy chỉnh">
                        <Palette className="w-4 h-4" />
                     </button>
                     <div className="bg-gray-100 p-1 rounded-lg flex text-xs font-bold">
                        <button 
                           onClick={() => setSubject(SubjectEnum.MATH)}
                           className={`px-3 py-1 rounded-md transition-all ${subject === SubjectEnum.MATH ? 'bg-white shadow text-blue-600' : 'text-gray-500'}`}
                        >Toán</button>
                        <button 
                           onClick={() => setSubject(SubjectEnum.LITERATURE)}
                           className={`px-3 py-1 rounded-md transition-all ${subject === SubjectEnum.LITERATURE ? 'bg-white shadow text-rose-600' : 'text-gray-500'}`}
                        >Văn</button>
                         <button 
                           onClick={() => setSubject(SubjectEnum.ENGLISH)}
                           className={`px-3 py-1 rounded-md transition-all ${subject === SubjectEnum.ENGLISH ? 'bg-white shadow text-indigo-600' : 'text-gray-500'}`}
                        >Anh</button>
                     </div>
                 </div>
              </div>
              ...
              <div className="mb-4">
                 <div className="bg-gradient-to-r from-pink-500 to-rose-500 rounded-xl p-4 text-white flex items-center justify-between shadow-lg cursor-pointer transform transition-transform hover:scale-[1.02]"
                      onClick={() => onNavigate(AppRoute.GAME_PLAY, { type: GameType.MIXED_CHALLENGE, mode: GameMode.STANDARD })}
                 >
                    <div className="flex items-center">
                       ...
                       <div>
                          ...
                          <p className="text-pink-100 text-sm">Toán, Văn & Anh • Độ khó tăng dần</p>
                       </div>
                    </div>
